...
#incorporo un cache para no perder los resultados. En caso de cambiar las semillas borrar y volver a correr
archivo_cache <- "./exp/HT2020/dataset_cache_binario2.txt"

if (file.exists(archivo_cache)) {
    dataset_cache <- fread(archivo_cache)
    #imprimir un head del dataset_cache para ver que se cargo correctamente
    print(head(dataset_cache))
} else {
    dataset_cache <- data.table(cp = numeric(), max_depth = integer(), min_split = integer(), min_bucket = integer(), ganancia_promedio = numeric())
}
...
rangos_max_depth <- c(7, 8, 9, 10, 11, 12)
    ...
    for (vmin_bucket in rangos_min_bucket) {
      if (vmin_split >= 2 * vmin_bucket) {
        for (vmax_depth in rangos_max_depth) {
          ...
          # Verificar si ya se ha calculado la ganancia promedio para esta combinación de parámetros
          params <- list(cp = vcp, max_depth = vmax_depth, min_split = vmin_split, min_bucket = vmin_bucket)
          resultado_cache <- dataset_cache[cp == params$cp & max_depth == params$max_depth & min_split == params$min_split & min_bucket == params$min_bucket, .(ganancia_promedio)]
          ...
          if (nrow(resultado_cache) > 0) {
              ...
              # Añade el nuevo resultado al cache para futuras ejecuciones
              nuevo_resultado <- data.table(
                  cp = params$cp,
              ...
              dataset_cache <- rbind(dataset_cache, nuevo_resultado)
          }
